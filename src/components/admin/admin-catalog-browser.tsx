      ...
      <header className="admin-page-header">
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3">
            <h1 className="admin-page-title mt-0" id="catalog-browser-heading">Products</h1>
            <span className="admin-source-badge">Square · read only</span>
          </div>
          <p className="admin-lede">
            Review the synchronized catalog and prepare products for the website. Prices and inventory stay in Square.
          </p>
        </div>
        <div className="admin-page-header-actions">
          <Link className="admin-button" href="/admin/product-placement#products">Open catalog publishing</Link>
        </div>
      </header>

      <section aria-labelledby="catalog-browser-heading" className="admin-panel admin-products-panel">
...
function ProductTableRow({ record }: { record: CatalogRecord }) {
  ...
  return (
    <tr>
      ...
      <td><WebsiteSetupBadge saved={record.saved} /></td>
    ...
    <article className="admin-product-mobile-card">
      ...
      <div className="min-w-0">
        ...
        <div className="admin-product-mobile-details">
          ...
          <WebsiteSetupBadge saved={record.saved} />
        </div>
      </div>
...
function WebsiteSetupBadge({ saved }: { saved: boolean | undefined }) {
  return saved
    ? <span className="admin-status-badge admin-status-badge--good">Configured</span>
    : <span className="admin-status-badge admin-status-badge--warning">Needs setup</span>;
}
