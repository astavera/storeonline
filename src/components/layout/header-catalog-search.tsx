...
export function HeaderCatalogSearch({ label = "Search" }: { label?: string }) {
  ...
  return (
    <span className="relative z-30" data-header-nav-id="search" ref={compactSearchRef}>
      <button
        ...
        type="button"
      >
        {compactSearchOpen ? <X aria-hidden="true" size={24} /> : <Search aria-hidden="true" size={24} />}
