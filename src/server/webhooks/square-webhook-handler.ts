...
import { confirmCompletedCapacityPayment } from "@/server/webhooks/capacity-payment-confirmation";
...
export type SquareWebhookOperations = {
  ...
  confirmCompletedCapacityPayment?(paymentId: string): Promise<void>;
  ...
  return async function squareWebhookHandler(record: WebhookInboxRecord) {
    ...
    if (record.eventType === "payment.updated") {
      ...
      if (payload.data.object.payment.status === "COMPLETED") {
        ...
        await operations.confirmCompletedCapacityPayment?.(payload.data.object.payment.id);
      }
      return;
    }
...
const productionOperations: SquareWebhookOperations = {
  ...
  async confirmCompletedCapacityPayment(paymentId) {
    await confirmCompletedCapacityPayment(paymentId);
  },
