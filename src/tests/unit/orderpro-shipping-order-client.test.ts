...
describe("OrderPRO Shipping order client", () => {
  it("requests canonical shipping availability with the quote identity headers", async () => {
    ...
    const client = createOrderProShippingOrderClient({
      config: { baseUrl: "https://orderpro.example.com", sharedSecret: "s".repeat(32) },
      fetchImpl: fetchMock as typeof fetch
    });
    const identity = orderProShippingCommandIdentity("quote", "store-3rd-avenue", "cart-hash");
    const checkoutItems = [{ squareVariationId: "variation-a", quantity: 1, source: "storefront" }];
  ...
  it("never enables the shared-secret fallback in production", () => {
    expect(getOrderProShippingOrderConfiguration({
      ORDERPRO_INTEGRATION_ENVIRONMENT: "PRODUCTION",
      ORDERPRO_STOREFRONT_FULFILLMENT_BASE_URL: "https://orderpro.example.com",
  ...
  it("uses M2M bearer auth and keeps command identity out of the body", async () => {
    ...
    const identity = orderProShippingCommandIdentity("release", "checkout-attempt-1", "expired");
    ...
    expect(JSON.parse(String(init?.body))).toEqual({ shippingOrderId: order.id, reason: "ABANDONED" });
  });
