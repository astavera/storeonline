...
describe("CartClient", () => {
  ...
  it("shows a concise empty state with a useful next step", async () => {
    ...
    expect(screen.getByAltText("Sad empty shopping basket").getAttribute("src")).toContain("empty-cart-sad-basket.png");
    expect(screen.getByText("Looks like you haven’t added anything to your cart yet.")).not.toBeNull();
    expect(screen.queryByText("Choose something from the online catalog when you’re ready.")).toBeNull();
