...
try {
  const { auditWebsiteMerchandisingPublication, publishWebsiteMerchandising } = await import("@/server/admin/website-merchandising-publication");
  const result = apply
    ? await publishWebsiteMerchandising(confirmation)
    : await auditWebsiteMerchandisingPublication();
  console.log(JSON.stringify({
    mode: apply ? "website-merchandising-publication-apply" : "website-merchandising-publication-audit",
  ...
  console.error(JSON.stringify({
    mode: apply ? "website-merchandising-publication-apply" : "website-merchandising-publication-audit",
    ok: false,
    error: describeError(error, "Website merchandising publication failed.")
  }, null, 2));
  process.exitCode = 1;
}
